...
TEST_CASE("get_unique_string", tag)
{
    ...
    std::string s0_result{
        get_unique_string(*id0, unique_string_purpose::RESULT)};
    std::string s0_request{
        get_unique_string(*id0, unique_string_purpose::REQUEST)};

    REQUIRE(s0_result == s0);
    REQUIRE(s0_result != s0_request);
}
