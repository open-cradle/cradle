...
digest_of(blob const& content)
{
    ...
    // digests stay consistent across the codebase (FR-9, FR-13).
...
{
    // Hashing happens here, outside the CAS (FR-13).
...
{
    // Caller-supplied digest; a no-op when already present (FR-11).
...
{
    // Recompute and compare; a mismatch is a trust-boundary fault (D4, FR-12).
