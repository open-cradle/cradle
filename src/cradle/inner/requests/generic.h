...
caching_level_type::
        none&& requires { std::same_as<decltype(Req::introspective), bool>; }
...
concept IntrospectiveRequest
    = CachedRequest<Req> && Req::introspective
      && requires(Req const& req) {
             {
...
template<typename Ctx, typename Req>
concept FullyCachedContextRequest
    = CachedContext<Ctx> && FullyCachedRequest<Req>
      && MatchingContextRequest<Ctx, Req>;

template<typename Ctx, typename Req>
concept IntrospectiveContextRequest
    = IntrospectiveContext<Ctx> && IntrospectiveRequest<Req>
      && MatchingContextRequest<Ctx, Req>;
