#ifndef CRADLE_THINKNODE_ASYNC_CONTEXT_H
#define CRADLE_THINKNODE_ASYNC_CONTEXT_H
 ...
 * machine.
 */
class root_local_async_thinknode_context final
...
/*
 * Recursively creates subtrees of non_root_local_async_thinknode_context
 * objects, with the same topology as the corresponding request subtree.
 *
 * A non_root_local_async_thinknode_context object will be created for each
 * request in the tree, but also for each value: the resolve_request() variant
 * resolving a value requires a context argument, even though it doesn't access
 * it.
 */
...
/*
 * Tree-level context, shared by all root_proxy_async_thinknode_context and
 * non_root_proxy_async_thinknode_context objects in the same context tree
 * (relating to the same root request), in the "testing" domain;
 * owned by the root_proxy_async_thinknode_context object.
 *
 * Note that an object of this class should not be re-used across multiple
 * context trees.
 */
class proxy_async_thinknode_tree_context : public proxy_async_tree_context_base
{
 public:
    proxy_async_thinknode_tree_context(
 ...
 */
class root_proxy_async_thinknode_context final
    : public root_proxy_async_context_base,
      public test_params_context_mixin
{
 public:
 ...
 private:
    std::unique_ptr<proxy_async_thinknode_tree_context> owning_tree_ctx_;
...
static_assert(ValidFinalContext<root_proxy_async_thinknode_context>);

/*
 * Context that can be used to asynchronously resolve non-root requests in the
 * "testing" domain, on a remote machine.
 */
class non_root_proxy_async_thinknode_context final
    : public non_root_proxy_async_context_base
{
 public:
    non_root_proxy_async_thinknode_context(
...
static_assert(ValidFinalContext<non_root_proxy_async_thinknode_context>);
...
// The async_thinknode_context functionality is limited: it can be passed to
...
class async_thinknode_context final : public root_local_async_context_intf,
                                      ...
                                      public test_params_context_mixin
{
 ...
 private:
    ...
    std::shared_ptr<root_local_async_thinknode_context> local_root_;
    // remote_root_ used only when proxy_name_ is non-empty;
    // remote_root_ exclusively owned by this object
    std::unique_ptr<root_proxy_async_thinknode_context> remote_root_;
...
static_assert(ValidFinalContext<async_thinknode_context>);
